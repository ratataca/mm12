...
def predict():
    if request.method == "POST":
        ...
        result = jsonify(PhotovleService.predict(user_id=user_id))
        print("예측하기")
        print(result)
        return result
...
def get_video():
    if request.method == "POST":
        ...
        print("동영상 저장 중 .....")
        user_id = str(request.json["user_id"])
        access_folder = os.path.join(os.getenv("TEMP_DATA_PATH"), user_id,"split_img")
        predict_folder = os.path.join('fake_labe_folder')

        total_count = len(glob.glob(access_folder+"/*.png"))
        os.makedirs(os.path.join(os.getenv("TEMP_DATA_PATH"), user_id,"pred_img"),exist_ok=True)
        ...
        for idx in range(1, total_count+1):
            ...
            cv2.imwrite(os.path.join(os.getenv("TEMP_DATA_PATH"), user_id,"pred_img",filename), result_img)
